...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/TR/WD-xsl">
  <xsl:template match="/">
	...
	<xsl:attribute name="STYLE">size:
                <xsl:choose>
                  <xsl:when test="bprice[. $le$ 300]">40px</xsl:when>
                  <xsl:when test="bprice[. $le$ 500]">40px</xsl:when>
                  <xsl:otherwise>20px</xsl:otherwise>
                </xsl:choose>
        </xsl:attribute>
                                          
            <TD>
		<xsl:if test="name[.='Victor Hugo' $or$ .='Leo Tolstoy']"><xsl:value-of select="name"/></xsl:if>
	    </TD>
            <TD>
